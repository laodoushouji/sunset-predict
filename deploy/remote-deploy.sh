...
curl -fsS http://127.0.0.1:3003/sunset | "$node_bin" -e "let s='';process.stdin.on('data',d=>s+=d);process.stdin.on('end',()=>{const j=JSON.parse(s);const m=j.metrics;const b=j.blueHour;if(j.spot!=='xihu'||typeof j.rawQuality!=='number'||j.rawQuality!==j.quality||typeof j.probability!=='number'||typeof j.verdict!=='string'||j.modelVersion!=='quality-v3.1'||j.source!=='xihu-model-v3'||j.timeOffsetMinutes!==-15||j.sourceStatus?.waqi!=='connected'||j.days?.length!==3||j.days.some(d=>typeof d.rawQuality!=='number'||typeof d.probability!=='number'||d.blueHour?.source!=='suncalc-2.0.1')||b?.source!=='suncalc-2.0.1'||typeof b.score!=='number'||!/^\d{2}:\d{2}$/.test(b.start)||!/^\d{2}:\d{2}$/.test(b.end)||![m?.cloudLow,m?.cloudMid,m?.cloudHigh,m?.visibilityKm,m?.windowTransparency].every(Number.isFinite))process.exit(1)})"
...
curl -fsS http://127.0.0.1:3003/sunset | "$node_bin" -e "let s='';process.stdin.on('data',d=>s+=d);process.stdin.on('end',()=>{const j=JSON.parse(s);if(j.sourceStatus?.qweather!=='connected'||j.weather?.source!=='qweather'||typeof j.weather?.label!=='string'||typeof j.weather?.kind!=='string'||(j.weather.blocksSunset&&(j.quality!==0||j.probability!==0)))process.exit(1)})"
curl -fsS http://127.0.0.1:3003/api/spot/waitan | "$node_bin" -e "let s='';process.stdin.on('data',d=>s+=d);process.stdin.on('end',()=>{const j=JSON.parse(s);const m=j.metrics;const b=j.blueHour;if(j.spot!=='waitan'||typeof j.rawQuality!=='number'||j.rawQuality!==j.quality||typeof j.probability!=='number'||typeof j.verdict!=='string'||j.modelVersion!=='quality-v3.1'||j.source!=='waitan-model-v4'||j.sourceStatus?.waqi!=='connected'||j.sourceStatus?.precipitation!=='connected'||b?.source!=='suncalc-2.0.1'||typeof b.score!=='number'||typeof j.weather?.label!=='string'||![m?.cloudLow,m?.cloudMid,m?.cloudHigh,m?.visibilityKm,m?.windowTransparency,m?.precipitationRateMmH,m?.precipitationProbability,m?.weatherCode].every(Number.isFinite)||(j.weather.blocksSunset&&(j.quality!==0||j.probability!==0)))process.exit(1)})"
curl -fsS http://127.0.0.1:3003/api/timeline -o /tmp/sunset-v2-timeline.json
"$node_bin" -e "const j=require('/tmp/sunset-v2-timeline.json');const today=j.days?.find(d=>d.offset===0);if(!/^\d{4}-\d{2}-\d{2}$/.test(j.today)||j.days?.length<3||today?.xihu?.spot!=='xihu'||today?.waitan?.spot!=='waitan'||today?.spots?.length!==8||today.xihu.modelVersion!=='quality-v3.1'||typeof today.xihu.rawQuality!=='number')process.exit(1)"
...
grep -q "js/app.js?v=20260726-spot-messages-v32" /tmp/sunset-v2-public-index.html
...
curl -fsS https://sunsetpredict.cloud/health | "$node_bin" -e "let s='';process.stdin.on('data',d=>s+=d);process.stdin.on('end',()=>{const j=JSON.parse(s);if(j.ok!==true||!j.services?.includes('feedback-v3')||!j.services?.includes('qweather-weather-v1')||!j.services?.includes('blue-hour-v1'))process.exit(1)})"
