...
curl -fsS http://127.0.0.1:3003/sunset | "$node_bin" -e "let s='';process.stdin.on('data',d=>s+=d);process.stdin.on('end',()=>{const j=JSON.parse(s);const m=j.metrics;if(j.spot!=='xihu'||typeof j.quality!=='number'||typeof j.probability!=='number'||typeof j.verdict!=='string'||j.source!=='xihu-model-v2'||j.timeOffsetMinutes!==-15||j.sourceStatus?.waqi!=='connected'||j.days?.length!==3||j.days.some(d=>typeof d.probability!=='number')||![m?.cloudLow,m?.cloudMid,m?.cloudHigh,m?.visibilityKm,m?.windowTransparency].every(Number.isFinite))process.exit(1)})"
...
curl -fsS http://127.0.0.1:3003/api/spot/waitan | "$node_bin" -e "let s='';process.stdin.on('data',d=>s+=d);process.stdin.on('end',()=>{const j=JSON.parse(s);const m=j.metrics;if(j.spot!=='waitan'||typeof j.quality!=='number'||typeof j.probability!=='number'||typeof j.verdict!=='string'||j.source!=='waitan-model-v3'||j.sourceStatus?.waqi!=='connected'||j.sourceStatus?.precipitation!=='connected'||typeof j.weather?.label!=='string'||![m?.cloudLow,m?.cloudMid,m?.cloudHigh,m?.visibilityKm,m?.windowTransparency,m?.precipitationRateMmH,m?.precipitationProbability,m?.weatherCode].every(Number.isFinite)||(j.weather.blocksSunset&&(j.quality!==0||j.probability!==0)))process.exit(1)})"
curl -fsS http://127.0.0.1:3003/api/timeline -o /tmp/sunset-v2-timeline.json
"$node_bin" -e "const j=require('/tmp/sunset-v2-timeline.json');if(!/^\d{4}-\d{2}-\d{2}$/.test(j.today)||j.days?.length<3||j.days.find(d=>d.offset===0)?.xihu?.spot!=='xihu'||j.days.find(d=>d.offset===0)?.waitan?.spot!=='waitan'||j.days.find(d=>d.offset===0)?.spots?.length!==8)process.exit(1)"
...
curl -fsS http://127.0.0.1:3003/api/spot/jingshan | "$node_bin" -e "let s='';process.stdin.on('data',d=>s+=d);process.stdin.on('end',()=>{const j=JSON.parse(s);if(j.spot!=='beijing'||j.source!=='beijing-model-v2')process.exit(1)})"
...
grep -q "20260717-feedback-v20" /tmp/sunset-v2-public-index.html
...
curl -fsS https://sunsetpredict.cloud/api/spot/jingshan | "$node_bin" -e "let s='';process.stdin.on('data',d=>s+=d);process.stdin.on('end',()=>{const j=JSON.parse(s);if(j.spot!=='beijing'||j.source!=='beijing-model-v2')process.exit(1)})"
