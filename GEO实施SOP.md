...
| `llms.txt` | **200（已上线）** | ✅ `frontend/llms.txt` 随前端发布，AI 爬虫站点地图就位 |
| 结构化数据 | 已实现 `FAQPage` + 落地页 `Dataset`/`Article`/`Organization` JSON-LD | ✅ 已是机器可读 Q&A + 预测数据集，已扩展 |
| 城市/省份落地页 | 19 城 + 18 省 `/spots/*`（共 37 个） | ✅ 天然匹配长尾问题（"杭州今晚有晚霞吗"） |
| 原创数据 | 每日晚霞/火烧云概率预测 | ✅ AI 最爱引用的独家数据集 |
| 每日新鲜度 | 数据每日更新 | ✅ "今日/实时"是 AI 偏好信号 |
| 站长验证 | GSC 静态验证文件已通过；Bing 已验证 | ✅ 间接提升实体可信度 |
| sitemap 提交 | 验证文件已 OK；GSC/百度 sitemap 仍未提交 | ⚠️ 影响传统收录，间接影响 AI 取源 |
| 页脚引用授权 | 全站可见「欢迎注明出处引用」 | ✅ 降低 AI/媒体引用顾虑 |

**结论**：GEO 技术基础已全面落地（llms.txt + /about 方法论页 + JSON-LD 扩展 + 页脚引用声明 + 37 个落地页 + 直给摘要 + FAQ 直给化）。核心短板转为**外部权威引用不足（P2）** + **站长平台 sitemap 提交待办（运维）**。
...
- [x] **P0** 创建并部署 `frontend/llms.txt`（2026-08-13 上线）
- [x] **P0** 确认 robots 放行 AI 爬虫（已满足，记录备查）
- [x] **P1** 城市/省份页加「今日直给摘要」区块（落地页已注入）
- [x] **P1** 建 `/about` 方法论页（定义 + 原理 + 术语 + 授权，2026-08-14 上线）
- [x] **P1** 扩展 JSON-LD（预测数据字段 + Article 权威信息，2026-08-14 上线）
- [x] **P1** FAQ 答案首句直给化
- [x] **P1** 页脚引用授权声明（全站可见，2026-08-14 上线）
- [ ] **P2** 知乎/小红书/百科 主动被引用
- [ ] **P2** 站外实体一致性
- [ ] **运维** GSC + 百度 提交 sitemap.xml（同时利传统 SEO）
- [x] **监控** seo-monitor 增 llms.txt / about 校验 + geo-stats；每周人工 AI 引用自检
